...
function TopProduct({ product, index = 0 }: { product: any; index?: number }) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.96 }}
      className="group relative flex flex-col h-full cursor-pointer overflow-hidden rounded-2xl bg-white shadow-[0_6px_18px_rgba(67,56,202,0.10)]"
    >
      {discountPct > 0 && (
      ...
      <div className="relative w-full overflow-hidden rounded-t-2xl bg-gradient-to-br from-[#EEF2FF] to-[#F8F9FB] aspect-square">
        <img
      ...
      <div className="px-2 py-1.5 flex flex-col gap-0.5">
        {Number(price) > 0 && (
          <div className="flex items-baseline gap-1 mt-0.5">
            <span className="text-[12px] font-extrabold text-[#4338CA] leading-none">${Number(price).toFixed(2)}</span>
...
function Banner({ slide }: { slide: Slide | undefined }) {
  ...
  return (
    <motion.div
      ...
      className="relative h-[132px] rounded-2xl overflow-hidden shadow-[0_10px_30px_rgba(67,56,202,0.28)]"
      style={{ background: 'linear-gradient(120deg,#4338CA 0%,#4F46E5 45%,#7C3AED 100%)' }}
    >
      {img ? (
...
export default function TopSales({ banner, products }: { banner?: Slide; products: any[] }) {
  ...
  return (
    <section className="mt-7 mx-3.5 rounded-3xl p-3.5 overflow-hidden shadow-[0_12px_32px_rgba(67,56,202,0.16)] relative"
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <motion.span
              className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] text-white flex items-center justify-center shadow-md"
              animate={{ rotate: [0, -8, 8, 0] }}
              transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
            >
              <Flame size={18} fill="currentColor" />
            </motion.span>
            <h2 className="text-[17px] font-extrabold text-[#4338CA] tracking-tight">Top Sales</h2>
            <motion.span
        ...
        <div className="relative mb-3 overflow-hidden rounded-full bg-white/70 border border-[#E0E7FF] py-1.5 px-3">
          <motion.div
            className="flex items-center gap-4 whitespace-nowrap text-[10.5px] font-bold text-[#4338CA]"
            animate={{ x: ['0%', '-50%'] }}
        ...
        <motion.button
          onClick={() => navigate(viewAll)}
          whileTap={{ scale: 0.97 }}
          className="mt-3 w-full flex items-center justify-center gap-1.5 py-2.5 rounded-2xl text-[13px] font-bold text-white shadow-[0_8px_20px_rgba(79,70,229,0.30)]"
