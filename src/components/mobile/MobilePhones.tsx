  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] font-sans text-[#0F172A] max-w-[480px] mx-auto pb-24">
          ...
          {categories.filter((c) => /phone|smartphone|mobile/i.test(c.name)).map((c) => {
            ...
            return (
              <button
                ...
                className="snap-start flex-shrink-0 w-[68px] flex flex-col items-center gap-1.5 active:scale-95 transition"
              >
                <div className={`w-[64px] h-[64px] rounded-full overflow-hidden flex items-center justify-center transition-all ${active ? 'ring-4 ring-[#6C3BFF]/20 scale-105 shadow-[0_6px_18px_rgba(108,59,255,0.30)]' : 'ring-1 ring-[#E5E7EB]'}`} style={active ? { background: 'linear-gradient(135deg,#6C3BFF,#4B2ECC)' } : { background: 'linear-gradient(135deg,#6C3BFF,#4B2ECC)' }}>
                  {img ? (
      ...
      <div className="flex items-center justify-between px-3.5 mt-3.5 mb-1">
        <p className="text-[12px] font-semibold text-[#64748B]">{loading ? 'Loading…' : `${products.length} product${products.length !== 1 ? 's' : ''} found`}</p>
      </div>
