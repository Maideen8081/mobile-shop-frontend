              ...
              return (
                <div key={`${it.productId}-${it.variantId ?? 'd'}`} className="flex items-center gap-2.5 py-2 border-b border-[#FEE2E6] last:border-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => updateQuantity(it.productId, -1)} aria-label="Decrease" className="w-7 h-7 rounded-full bg-[#FEE2E6] flex items-center justify-center active:scale-90 transition">
                      <Minus size={14} className="text-[#1F2937]" />
                    </button>
                    <span className="text-[13px] font-bold w-5 text-center">{it.quantity}</span>
                    <button onClick={() => updateQuantity(it.productId, 1)} aria-label="Increase" className="w-7 h-7 rounded-full bg-[#FEE2E6] flex items-center justify-center active:scale-90 transition">
